...
test("install atomically precaches the exact versioned shell", async () => {
    ...
    assert.equal(expectedShellAssets.length, 55);
    assert.equal(
    ...
    assert.deepEqual(
        harness.calls.addAll[0].map(request => (
            new URL(request.url).pathname
        )),
        expectedShellAssets
    );
    assert.equal(
        harness.calls.addAll[0].every(
...
test("v0.7.5 c1 shell updates to the current v0.8.4 release shell", async () => {
    ...
    assert.equal(
        harness.calls.addAll[0].some(
            request => new URL(request.url).pathname === "/pages/recovery.js"
        ),
        true
    );
    assert.deepEqual(
